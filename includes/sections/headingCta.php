...
<section class="relative bg-neutral/5<?= $rootClass ?>"<?= $rootAttr ?>>
  <div class="py-40px lg:py-[120px] wrapper">
    <div class="flex items-center gap-8px">
      ...
      <p class="text-16px/1_6 lg:text-20px/1_5"
      ><?= $label ?><?php if ($link): ?>
        <a
          href="<?= $link['url'] ?>"
          target="<?= $link['target'] ?? '_blank' ?>"
          class="font-medium hover:text-neutral-red transition-colors"
        ><?= $link['title'] ?></a>
      <?php endif; ?></p>
    </div>
    <div class="mt-24px flex flex-col lg:flex-row lg:items-end justify-between gap-24px lg:gap-32px">
